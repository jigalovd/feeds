...
`main`. Актуальный clean-context `PASS` хранится в verdict и PR evidence
текущего head, а не кодируется статусом этого плана. Канонический
implementation-профиль определён в карте документации и engineering workflow.
Остальные package paths и test targets появляются только вместе с требующим их
срезом и не предполагаются планом заранее.
...
- `CaptureBatch` является закрытым union `CapturingBatch | CompleteBatch` по
  полю `status`;
- оба варианта фиксируют непустые непрозрачные `batch_id`, `run_id` и
  `stream_id`, нижнюю и верхнюю границы, а также UTC-момент открытия batch;
- обычная нижняя граница и всегда числовая верхняя граница используют
  неотрицательный message cursor; `0` является sentinel пустой истории, а
  bootstrap использует явную временную нижнюю границу;
- `capturing` не является завершённым входом дальнейшей обработки;
- `capturing` не имеет поля элементов, а только `complete` предоставляет
  immutable-последовательность `ContentItem`; пустой complete batch допустим;
- complete-элементы совпадают с batch по `batch_id` и `stream_id`, имеют
  уникальные `item_id` и непересекающиеся origin message IDs;
- message IDs элементов находятся внутри зафиксированных числовых границ;
- верхняя граница `0` допускает только пустой complete batch;
- порядок complete-элементов сохраняется как результат capture без

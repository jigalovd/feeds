...
**Статус комплекта:** дизайн утверждён, реализация начата
...
| Реализовать PR-срез, закрыть milestone или исправить дефект | [workflow разработки](engineering/development-workflow.md), нормативный владелец поведения, применимый архитектурный сценарий | [активный план](plans/README.md) и технический владелец интеграции |
...
Документация определяет целевое поведение и дизайн. Созданы минимальный
Python-каркас и публичные контракты `ContentItem` и `CaptureBatch`; остальные
публичные контракты, предметные модули, CI и поставка ещё не существуют.

Канонический текущий implementation-контур:
...
Проверяющий Markdown-скрипт подтверждает только структуру документации и не
доказывает реализацию продукта.
